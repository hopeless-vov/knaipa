import { applyPostFetchFilters, applyClientRefinements } from '../utils/placeFilters';
...
describe('applyClientRefinements', () => {
  it('returns all places when no refinements are set', () => {
    expect(applyClientRefinements(MOCK_PLACES, filters({}))).toHaveLength(MOCK_PLACES.length);
  });

  it('filters by minimum review count', () => {
    const result = applyClientRefinements(MOCK_PLACES, filters({ minReviews: '200+' }));
  ...
  it('sorts by rating descending without mutating the input', () => {
    ...
    const result = applyClientRefinements(input, filters({ sort: 'rating' }));
  ...
  it('combines minReviews and rating sort', () => {
    const result = applyClientRefinements(MOCK_PLACES, filters({ minReviews: '200+', sort: 'rating' }));
